...
export default function NoticeUpdateScreen({ route }) {
  const { id, contents, titles } = route.params;
  const [title, setTitle] = useState(titles); // 제목 state
  ...
  const handlePost = () => {
    ...
    API.put(`/admin/notice/${id}`, postData, {
      headers: {
        Authorization: token,
      },
    })
