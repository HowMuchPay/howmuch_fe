...
import { getCenterPosition, getCenterPositionFromIndex, getIndexFromOffset, fillEmpty, asPickerFormat } from "../utils/utils";
import { MERIDIEM_ITEMS, MINUTE_ITEMS, HOUR_ITEMS, BUTTON_HEIGHT, GAP } from "../utils/values";
...
const TimePicker = ({ value, onChange, width, buttonHeight, visibleCount }) => {
  ...
  const animatedValues = React.useRef(Array.from({ length: 3 }).map(() => new Animated.Value(0)));
  ...
  const scrollProps = React.useMemo(() => {
    return ITEMS.map(({ key, items }, index) => getScrollProps(index, key, items));
  }, [dateString]);

  React.useEffect(() => {
    const meridiem = isPM(value) ? "오후" : "오전";
    const hour = String(isPM(value) ? value.getHours() - 12 : value.getHours()).padStart(2, "0");
    const minute = String(value.getMinutes()).padStart(2, "0");

    const matchIndex = [MERIDIEM_ITEMS.indexOf(meridiem), HOUR_ITEMS.indexOf(hour), MINUTE_ITEMS.indexOf(minute)];
  ...
  return (
    <View style={[styles.container, { width, height: visibleCount * buttonHeight }]}>
      {scrollProps.map((props, scrollViewIndex) => {
        ...
        return (
          <ScrollView {...props} onScroll={Animated.event([{ nativeEvent: { contentOffset: { y: props.animatedValue } } }], { useNativeDriver: false })}>
            {renderItems.map((item, index) => {
              const position = getCenterPositionFromIndex(props.items.indexOf(item));

              const opacity = props.animatedValue.interpolate({
                inputRange: [position - BUTTON_HEIGHT, position, position + BUTTON_HEIGHT],
                outputRange: [0.3, 1, 0.3],
                extrapolate: "clamp",
              });

              return <Button key={item} style={{ opacity }} label={item} onPress={props.getOnPress(item)} />;
            })}
          </ScrollView>
        );
      })}
